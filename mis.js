...
document.getElementById('request-form').addEventListener('submit', function(event) {
	...
	const q = document.getElementById('reqProg').value;
...
function startPolling(requestId) {
	pollingInterval = setInterval(() => {
		fetch('get_answer.php?request_id=' + requestId)
			.then(response => response.json())
			.then(data => {
				if (data.answer) {
	...
	setTimeout(() => {
		...
		const answerCheck = document.getElementById('answer-text');
		if (answerCheck && answerCheck.textContent.trim() === '') { 
			alert('Answer empty. Please check the VAULT.');
		}
	}, 120000); // 2 minutes
...
function calculateFaceMove(e) {
	var 	
		carPos = prog.selectionEnd,
		div = document.createElement('div'),
		span = document.createElement('span'),
		copyStyle = getComputedStyle(prog),
		caretCoords = {}
	;
	if(carPos == null || carPos == 0) {
		// if browser doesn't support 'selectionEnd' property on input[type="prog"], use 'value.length' property instead
		carPos = prog.value.length;
	}
	...
	div.textContent = prog.value.substr(0, carPos);
	span.textContent = prog.value.substr(carPos) || '.';
	div.appendChild(span);
	
	if(prog.scrollWidth <= progScrollMax) {
		caretCoords = getPosition(span);
		dFromC = screenCenter - (caretCoords.x + progCoords.x);
		eyeLAngle = getAngle(eyeLCoords.x, eyeLCoords.y, progCoords.x + caretCoords.x, progCoords.y + 25);
		eyeRAngle = getAngle(eyeRCoords.x, eyeRCoords.y, progCoords.x + caretCoords.x, progCoords.y + 25);
		noseAngle = getAngle(noseCoords.x, noseCoords.y, progCoords.x + caretCoords.x, progCoords.y + 25);
		mouthAngle = getAngle(mouthCoords.x, mouthCoords.y, progCoords.x + caretCoords.x, progCoords.y + 25);
	} else {
		eyeLAngle = getAngle(eyeLCoords.x, eyeLCoords.y, progCoords.x + progScrollMax, progCoords.y + 25);
		eyeRAngle = getAngle(eyeRCoords.x, eyeRCoords.y, progCoords.x + progScrollMax, progCoords.y + 25);
		noseAngle = getAngle(noseCoords.x, noseCoords.y, progCoords.x + progScrollMax, progCoords.y + 25);
		mouthAngle = getAngle(mouthCoords.x, mouthCoords.y, progCoords.x + progScrollMax, progCoords.y + 25);
	}
...
function onProgInput(e) {
	...
	curProgIndex = value.length;
	
	// very crude prog validation to trigger effects
	if(curProgIndex > 0) {
...
function onProgBlur(e) {
	activeElement = null;
	setTimeout(function() {
		if(activeElement == "prog") {
		} else {
			if(e.target.value == "") {
...
function initLoginForm() {
	...
	// handle events for prog input
	prog.addEventListener('focus', onProgFocus);
	prog.addEventListener('blur', onProgBlur);
	prog.addEventListener('input', onProgInput);
	progLabel.addEventListener('click', onProgLabelClick);
	...
	progScrollMax = prog.scrollWidth;
